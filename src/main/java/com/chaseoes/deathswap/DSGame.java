...
import com.chaseoes.deathswap.utilities.PlayerStateStorage;
import com.chaseoes.deathswap.utilities.SwapState;
...
public class DSGame {
    ...
    public void joinGame(Player player) {
        if (!queue.gameHasRoom() || state == GameState.INGAME) {
            queue.add(player);
            player.sendMessage(DeathSwap.format("You are number " + queue.getPosition(player) + " in the queue"));
            ...
            if (DeathSwap.getInstance().getMap(name).getType() == GameType.PRIVATE) {
                ...
                player.sendMessage(DeathSwap.format("Successfully joined the map " + name + "!"));
                broadcast(DeathSwap.format(player.getName() + " has joined the game!"));
                if (players.size() == 2) {
                    startGame();
                }
            ...
            player.sendMessage(DeathSwap.format("That game is currently rolling back."));
        }
        sign.update();
    }
    ...
    public void leaveGame(Player player) {
        ...
        broadcast(DeathSwap.format(player.getName() + " has left the game."));
    ...
    public void winGame(Player player) {
        broadcast(DeathSwap.format(player.getName() + " won on the map " + name + "!"));
        stopGame();
    }

    public void stopGame() {
        broadcast(DeathSwap.format("The game has ended."));
        stopSwapTask();
    ...
    public void startGame() {
        broadcast(DeathSwap.format("The game has started! Good luck!"));
        startSwapTimer();
            ...
            swapId = Bukkit.getScheduler().runTaskTimer(DeathSwap.getInstance(), new Runnable() {
                ...
                public void run() {
                    if (currTime > currRand) {
                        broadcast(DeathSwap.format("Commencing swap!"));
                        swap();
                        if (diff != 0) {
